...
## 사후 검증 (2026-09-14, SA 9/13 Why 4-hi HBM Wins)

SemiAnalysis 9/13「Long Live the Short King: Why 4-hi HBM Wins」(Myron Xie, Bryan Shan, Harrison Barclay 외). **페이월**: Memory Model 공급사·SKU·수요/공급 표 — 발명 금지.

- **주장**: HBM “더 높게·더 많이” 추세가 꺾임. 차세대는 12-hi 표준에서 **8-hi 표준**으로, 랩 ASIC·추론은 **4-hi**가 $/bandwidth·토큰당 비용 최적. 대역폭은 스택 높이와 무관(HBM4/4E 2048 I/O; 다이가당 최대 512 I/O → **4-hi가 풀대역 최소 단수**). 가격은 GB(용량) 중심이라 낮은 단수 = 거의 “공짜 점심”에 가까운 $/BW.
- **메커니즘**: (1) 워크로드 — pretrain=용량, RL≈추론, decode=대역폭; 컴퓨트 배분이 대역폭 민감으로 이동(Tokenomics). (2) 랙 스케일 — GB300 NVL72 집계 HBM이 H200 HGX 대비 ~**18×**; Kimi K3 2.8T MXFP4 ~**1,561GB**는 ~**21TB**의 **<8%**. Rubin Ultra **NVL576**(NVL72의 8×)가 GPU당 HBM **1/3 감소(288→192GB)**를 상쇄. (3) 루프라인(HBM4E·NVL576·K3): 최소 SLA ~105 tok/s/user에서 12-hi≈8-hi; ~213에서 4-hi 이상 무익. 최대 처리량 8-hi **+8%**, 12-hi **+10%** vs 4-hi. 시스템비 8-hi **+12.1%**, 12-hi **+26.3%** → 현 메모리 가격·NVDA 마진에선 높은 단수가 **토큰당 비용↑**. (4) KV offload·DeepSeek v4.1 Flash류(~KV **~75%** 축소 vs v4 Flash)가 용량 압박 추가 완화. (5) 4-hi면 동일 웨이퍼에서 큐브 **>2×**(vs 8-hi) → 병목이 HBM 웨이퍼→로직/기판/PCB/전력으로 이동, 남는 웨이퍼는 서버 DRAM 등으로.
- **회의론**: (1) 공급사 수익·제품 믹스 반응은 페이월. (2) 미래 가중치 3× 시나리오에선 8-hi가 일부 SLA에서 TCO 우위 가능 — 12-hi vs 8-hi는 SA 공개상 여전히 비매력. (3) 훈련·대배치 RL은 용량 민감 잔존. (4) 실측 대역폭≪피크라 곡선은 좌측 이동 — 방향은 같음.
- **2–3Q 반증조건**: NVDA/랩이 Rubin 이후 다시 12-hi+·고용량 SKU로 회귀 공시; HBM ASP/GB 급락으로 용량 오버프로비저닝 비용 소멸; AgentX에서 4-hi 제한이 고동시성 SLA를 깨뜨림.
- **투자 번역**: 세대 서사에 **“단수 경쟁 → 대역폭당 비용·웨이퍼당 토큰”** 축 추가. SK/삼성 “고단 양산” 뉴스만으로 비트·마진 확장을 단정하지 말 것.

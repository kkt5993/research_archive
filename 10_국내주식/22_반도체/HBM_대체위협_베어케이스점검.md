...
8/4 FMS: SK하이닉스×SanDisk가 HBF **첫 표준 스펙**을 OCP에 공개. 본문 '2026 하반기 시제품' 경로의 스펙 확정. 양산 채택은 여전히 2027 초 샘플 이후.


## 사후 검증 (2026-09-14) — DeepSeek V4.1-Flash KV (공식만)

[DeepSeek 공식 9/10](https://www.deepseek.com/en/news/deepseek-v4-1-flash/): 직전 세대 대비 KV cache가 **HBM 1/4 · SSD 1/8**. Agent 캐시 히트 비용 절감이 목적이고, **산업 HBM 비트 −75% 선언이 아님**. 본문 ④ 효율화 레인과 정합 — 토큰당 HBM은 줄고, 산업 비트는 **GPU 대수 × 스택**이 같이 줄어야 줄어듦. 모델 출시 스펙 재탕 금지.

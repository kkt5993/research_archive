...
## 🟢 2026-09-01 갱신 — 효율화·오프로드가 실제로 깎는 것 (1차)

이 파일의 6개 후보 중 **④ 효율화**와 **③ HBF/플래시**가 2026년 하반기에 제품 문장으로 나왔다. GDDR7(①)은 Rubin **CPX**가 계속 담당(컨텍스트 GPU, 128 GB GDDR7).

| 우회 | 깎는 비트 | 안 깎는 것 | 출처 |
|---|---|---|---|
| DeepSeek MLA | KV **−93.3%**(V2 논문) | 학습 HBM, 가중치 | arXiv:2405.04434 |
| Llama 4 Scout 10M | 청크 어텐션 **8192**. 10M dense KV 아님 | 가중치 HBM | Meta Llama 4 |
| NVFP4 KV | FP8 대비 KV **최대 50%** | 학습 | NVIDIA Rubin 블로그 |
| 연산당 GB | Ultra 19.2 → Rubin **5.76 GB/PF** (288/50) | 세션 수 × 컨텍스트 | 산수 + nvidia.com |
| BlueField-4 ICMS | 긴 KV를 플래시로. “cannot be stored on GPUs long term” | 핫 프리픽스는 여전히 HBM | NVIDIA CES |
| Groq 3 LPX | SRAM **랙 128 GB** + DDR5 **12 TB**. HBM 대체 아님 | 어텐션/KV는 Rubin | nvidia.com/lpx |
...
훈련(training)은 성능이 전부라 비싼 HBM을 감수한다. 그런데 AI 연산의 무게중심이 **추론(inference, 이미 약 75%)**으로 넘어가면 계산이 달라진다:

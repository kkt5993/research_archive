...
## 🟢 2026-09-01 갱신 — NVHBM (NVIDIA 1차)

엔비디아가 표준 HBM4 옆에 **NVHBM**을 별도로 판다. Fusion 블로그: JEDEC **HBM4e 대비** 스택 대역 **+30%**, HBM 전력 **−15%**, 컨트롤러를 스택 베이스다이에 넣는 커스텀. 하이퍼스케일러 XPU용. 삼성 Q2'26 콜은 파운드리 쪽 **HBM 베이스다이 수요**를 따로 언급 — 이 파일의 “베이스다이가 파운드리로 넘어간다”와 정합.

표준 빈(12-hi 36 GB HBM4 볼륨)과 커스텀 퀄은 **같은 CoWoS 대기열이 아니다.** 점유 이동은 스택 수율이 아니라 **어느 고객의 베이스다이 도면을 쥐었는지**. 상세 세대표는 `HBM_완전정복_개념구조작동원리세대.md`.

**2026-09-01c.** [NVHBM 블로그 2026-08-26](https://blogs.nvidia.com/blog/nvlink-fusion-nvhbm-custom-high-bandwidth-memory/): 표준 **HBM4E 대비** 스택 BW **+30%**, HBM 전력 **−15%**, XPU 다이 면적 **+25%**. Annapurna **Trainium4**가 첫 작업. **2026 Rubin GPU-KV 드라이버 아님.**

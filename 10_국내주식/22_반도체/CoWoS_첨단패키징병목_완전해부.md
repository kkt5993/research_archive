...
## 🟢 2026-09-01 갱신 — 패키징 캘린더·커스텀 베이스다이 (회사 PR 1차)

> CoWoS wpm·리드타임 숫자는 이 파일의 TrendForce 교차검증을 **그대로**. 아래는 **메모리 3사·NVIDIA가 자기 입으로 찍은 후공정 일정**만 추가. SemiAnalysis 원문 재사용 금지.

### 붙이는 곳이 GPU 공장만이 아님

HBM 스택은 SK하닉·삼성·마이크론 후공정(TSV·MR-MUF/TC-NCF)을 거쳐야 CoWoS 식장에 입장한다. 2027–29 병목 후보는 TSMC CoWoS만이 아니라 **HBM 패키징 라인 자체**.

| 사이트 | 회사 말 | 비트가 나오는 때 |
|---|---|---|
| SK하닉 P&T7 (청주) | WT 2027-10 완료, **WLP 2028-02 완료** | 패키징. 2028 전에 WLP 풀가동 아님 |
| SK하닉 용인 1기 | 클린룸 **early 2027** | 프론트엔드. 오픈 ≠ 출하 |
| SK하닉 인디애나 | 클린룸 2028-10, 차세대 HBM MP **2029 2H**. 한국 웨이퍼 → 미국 패키지/테스트 | 2027–28 공급 아님 |
...
### Rubin이 식장 면적을 어떻게 쓰나

Rubin GPU 블로그: HBM4 **인터페이스 폭 2× vs HBM3e**, 12-hi, 최대 288 GB. 패키지당 스택 수는 Ultra와 같은 용량 클래스(288)라 **용량으로 CoWoS 자리를 더 먹는다기보다 대역·커스텀 PHY**. AMD MI455X는 **432 GB / 12 스택**(amd.com, 2027) — 경쟁 가속기는 스택 수를 더 올릴 수 있음.
...
| 종류 | 인터포저 | 특성 | 쓰임 |
|---|---|---|---|
| **CoWoS-S** | **실리콘 통판** | 가장 빠름·가장 비쌈. **레티클 한계(약 858mm²)** → 스티칭으로 약 3.3배(–2,700mm²)까지 확장하나 수율 급락 | H100 세대 주력 |

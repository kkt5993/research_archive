...
> **검증 메모(중요 — 개정 이력)**: v1은 "HBM3E 스팟–$300/스택(Silicon Analysts)"을 앵커로 써서 HBM3E 가격이 하락한다고 가정했다. 그러나 ① 이 수치는 2025년 산업 매출(약 $35B, Yole)과 나누기 검산이 안 맞고(평균 $16/GB여야 하는데 $8.3/GB), ② 삼성·SK가 **2026년 HBM3E 계약가를 약 20% 인상**했다는 보도(TrendForce/Digitimes, 2025.12)와 방향이 반대다. 디램 슈퍼사이클(1Q26 재래식 디램 계약가 +105–110% QoQ)까지 반영해 **가격 경로를 상향 재보정**했다. 2027 가격 앵커(Gb당 $4–5)는 오래된 수치가 아니라 **2026년 7월 12일자 보도**로 최신임을 재확인했다.

---

## 사후 검증 (2026-09-01)

SemiAnalysis(8/25): **Rubin Ultra HBM4는 192GB 8-hi**. 프리뷰 1TB·본문 GPU당 탑재량 **384GB(2027)** 가정과 불일치. 비트 성장 경로를 재작업하기 전엔 2027 물량·매출 상단을 쓰지 말 것. 엔비디아 공식 확인 전 주장.


## 사후 검증 (2026-09-14) — 레거시 DRAM 가격(한은) → HBM ASP 압력

한은 7월 수출물가 품목 서술: **DRAM 전년동월비 +270.3%**, 전월비 +6.6% ([보도자료](https://www.bok.or.kr/portal/bbs/B0000501/view.do?menuNo=201264&nttId=11063768)). 본문 “웨이퍼당 이익 DDR5>HBM → HBM 인상” 경로의 **공식 물가 앵커**. 스택 $/Gb·**$150B/2027 모델은 안 바꿈**. 상세 표는 `메모리_바텀업_레거시디램낸드_이익모델.md`.
...
| | 2025 | 2026 | 2027 협상 중 |
|---|---|---|---|
| **HBM3E** | 약 $500 | **약 $600 (+20% 인상)** | 유지~상승 (구세대라고 안 싸짐 — 공급 부족) |
| **HBM4** | — | 약 $580–700 (Gb당 약 $2) | **$1,150–1,450 (Gb당 $4–5, 2배+)** |
...
| 항목 | 수치 | 출처(날짜) |
|---|---|---|
| HBM3E 2025 계약가 | 12단 36GB–$500, 8단 24GB–$360 (GB당 약 $14–15) | 2024–25 업계 보도 종합 |
| HBM3E 2026 계약가 | **약 20% 인상** → 스택당 약 $600 (GB당 약 $17–18) | TrendForce/Digitimes (2025.12.24) |
| HBM3E 인상 모멘텀 | DDR5 수익성 상회가 "2026 HBM3E 가격을 강화" | TrendForce (2025.12.18) |
| HBM4 2H26 | **Gb당 약 $2** (GB당 $16, 스택당 약 $580) · 삼성 요구가 약 $700 | 서울경제(2026.7.12) · Bloomberg |
...
| 2026 출하량 | **30B Gb 이상 = 약 3.75EB** · 2026 캐파는 2025년 말 이미 완판 | TrendForce |
| 2025 출하량 | 약 2.2EB (+70% 역산, 추정) | — |
| 2027 출하량 | 약 5.25EB (**+40% — 가정**, 성장 둔화 추세 연장) | 검증된 전망치 아님 |
...
> 참고: 2026년 기본에서 HBM4($17)가 HBM3E($18)와 비슷한 건 오타가 아니다 — 신제품 초기 가격(Gb당 약 $2)이 확 인상된 구세대와 겹치는 과도기 현상으로, 2027년에 프리미엄이 크게 벌어지는 구조다(서울경제 보도 기준).
...
1. **2025 검산**: 모델 $31B vs Yole 약 $35B (-10%, 정합) ✅
...
4. **GPU 한 대 검산**: Rubin 288GB = 36GB×8스택. 2027 기본 가격($1,150/스택)이면 **GPU 한 대에 HBM만 약 $9,200** — GPU 판가(3–5만 달러 추정)의 20–30%. 엔비디아가 장기공급 딜($500B)로 물량을 잠근 이유이자, 동시에 **탑재량 절약 설계(HBF·GDDR7 계층화, ☞ 대체위협 리포트)의 유인이 커지는 임계점** ✅
...
## 사후 검증 (2026-09-14, SA 9/13 4-hi HBM — 바텀업 함의)

SemiAnalysis 9/13 Short King. Memory Model 표 페이월. 기존 8/25 주석(Rubin Ultra **192GB 8-hi**, 본문 384GB 가정 충돌)과 **정합·심화**.

- **주장**: 추론 최적 SKU가 4-hi로 기울면 **스택당 GB·ASP 경로**와 **웨이퍼당 큐브 수**가 동시에 움직임 — 단순 “단수↑→매출↑” 바텀업은 상단 편향. 공개: 8-hi 시스템비 **+12.1%**, 12-hi **+26.3%** vs 4-hi인데 처리량 증분은 그 아래 → $/token 악화.
- **메커니즘**: 대역폭/큐브 불변·과금은 GB → 낮은 단수 = $/BW↓. 큐브 harvest >2×(4-hi vs 8-hi)면 동일 HBM 웨이퍼로 더 많은 가속기 실장 가능(로직·CoWoS·전력이 새 병목). KV offload·모델 측 KV 축소가 용량 임계를 추가로 낮춤.
- **회의론 / 모델 가드**: (1) 공급사 믹스·가격·출하 큐브 전망은 페이월 — **2027 매출 상단 재작성 금지**. (2) 훈련용 고용량 SKU·HS 커스텀은 잔존 → 산업 전체가 4-hi로 붕괴한다는 가정 금지. (3) Rubin Ultra 192GB는 이미 반영 대기 중인 하향 요인; 4-hi는 **그다음** 옵션.
- **2–3Q 반증조건**: 2027 HBM 계약이 고단·고GB ASP로 재고정; 출하 믹스에서 12-hi 비중이 재상승; 웨이퍼 제약 해소로 용량 경쟁 재개.
- **투자 번역**: 바텀업은 **GPU당 GB 가정·스택 높이 믹스**를 명시적으로 분리해 추적. “HBM 슈퍼사이클 = 무조건 고단” 서사 약화. 피크/사이클 관찰은 AI_Capex **2027Q1**과 별축.

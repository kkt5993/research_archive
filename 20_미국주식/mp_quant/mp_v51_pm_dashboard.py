...
L += ['## 1. 유동성 — AUM 규모별 청산 소요일수\n',
      '일평균 거래대금(ADV) 대비 **참여율 20%** 가정. 전량 청산 기준.\n',
      '| 운용규모 | 포트 가중평균 | 최장 종목 | 3일 초과 종목수 | 5일 초과 |',
      '|---|--:|--:|--:|--:|']
for aum_e in [1000, 5000, 10000, 30000, 50000]:
    aum_usd = aum_e * 1e8 / FX
    pos = aum_usd * d.mp_weight / 100
    dtl = pos / (d.adv_usd * 0.20)
    L.append(f'| {aum_e:,}억원 (${aum_usd/1e6:,.0f}M) | {float((w*dtl).sum()):.2f}일 | '
             f'{dtl.max():.1f}일 ({dtl.idxmax()}) | {int((dtl>3).sum())} | {int((dtl>5).sum())} |')
L.append('\n**유동성은 제약이 아니다.** 3조원 규모에서도 가중평균 1일 미만이며, '
         '커버리지 20명+ 대형주만 담은 결과다.')
...
L += ['\n**최대 편출 10종목**\n', '| 종목 | 변동 |', '|---|--:|']
for t, v in sells.head(10).items(): L.append(f'| {t} | {v:+.2f}%p |')

...
TILT = {
    ...
    # --- 커뮤니케이션 ---
    'META': 1.5, 'NFLX': 2.0,
    # GOOGL: 0.4 → 0.7. **UW를 완화한다.** 성장 −28%는 기저효과였고(2년 CAGR +17.2%),
    # 영업마진 26%→32%·자체 기대수익률 11.3%·컨센 +23.3%로 강한 UW를 지지하는 데이터가 없다.
    # v4.7부터 지적해온 "검색 잠식 논거 하나에 걸린 UW"를 데이터 쪽으로 되돌린 것이다.
    'GOOGL': 0.7,
    # --- 경기소비 ---
    'AMZN': 1.2,
    # --- 금융: 은행 0, 통과세 OW ---

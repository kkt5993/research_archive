...
## 사후 검증 (2026-09-14, SA 9/11 Nvidia Backstop Universe)

SemiAnalysis 9/11「Nvidia’s Backstop Universe – Heads I Win, Tails Who Loses?」(Daniel Nishball, Oliver Kennon, Terence Ong). **페이월**: Compute/Capital/Markets Model 차트·전망 곡선 상세 결측 — 발명 금지. 공개 본문 수치만. 데스크 8-K PORTS-Pike($105B cap·4.25 IT-GW)와 **교차 확인·확장**.

- **주장**: 2Q F1/27 10-Q 기준 오프밸런스 보증·약정 총액 **$530B**(직전 분기 **$184B**). 온밸런스 부채 **$91B**(총부채 **$33.4B**, 그중 시니어노트 **$25B**)를 크게 상회. NVDA는 기가스케일러(IG 자본 게이트키퍼) 밖에서 네오클라우드·네오랩이 IG급으로 빌릴 수 있게 **대체 신용 앵커**를 제조. Heads = GPU 풀마진 매각 + 플로어 위 수익공유; Tails = 수요 붕괴 시 네오클라우드 무수익이지만 플로어로 대주 상환·전염 방파제. 동시 실패(네오클라우드 채무불이행 + NVDA 자체 현금창출력 붕괴)에서만 “모두 패배”.
- **메커니즘 (공개 라인아이템)**: (1) Supply/capacity commitments **$119B→$279B**(CFO: 주로 메모리, **96%가 F1/29까지**). (2) Guarantees·LPS **$3.5B→$108.5B** — SB Energy **PORTS-Pike**(오하이오) **4.25 GW**를 OpenAI에 20년 임대, NVDA가 land·power·shell 보증(데스크 8-K $105B cap과 정합). (3) 신규 **AI cloud agreements $36B** = AICP take-or-pay 플로어(GB300 평균 약 **$2.35/hr/GPU** vs 5년 시장 **$4.50–4.60**). 공개 딜: Firmus Batam 360MW **$21.1B**, SharonAI **$4.2B**, GMI 추정 **$2.2B**; SA는 AICP **일시 중단** 후 F1/27 마감 추정 **$51B**. (4) 신규 **datacenter leases $20B**(임차인으로 서명 후 제3자 재배정 의도). (5) 자산: cash **$12B→$22B**, investment book **$45B→$128B**, equity book **$99B**(상반기 평가익 **$23.7B**, 추가 약정 **$25B**). SA 카운트 현재 백스톱 용량 **~6.5 GW**(대부분 미건설). HS(MS/Meta/AWS/Oracle) 제3자 임대 ~**15 GW**(2026) → **>35 GW**(2028)가 암묵적 더 큰 백스톱. Residual-value(PE 파트너십, 딜당 최대 **~25%**): SA 공개 비교 AICP **~$59B/GW**, PORTS-Pike **~$25B/GW**, residual **~$9.4B/GW**(DC 레그는 미해결). 목표 자본 **>$500B**; SA 탐색 가정은 F1/31까지 펀딩 **$2.5T**·해당 라인 **$55B→$373B**(베이스케이스 아님 — 능력 탐색).

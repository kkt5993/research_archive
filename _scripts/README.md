...
삼성전자·SK하이닉스처럼 **장내 직접취득** 하는 회사의 매수는 KRX 투자자별 통계에서 '기타법인' 으로
잡힌다. 이 스크립트는 종목별 기타법인 순매수(금액·주식수)와 종가를 pykrx 로 받아 공시 예정주식수 대비
집행률·잔여·현재 속도로의 소진 예상일을 계산하고 `_manifest/데이터/자사주_소진여력_<날짜>.{csv,json}` 에 떨어뜨린다.
공시 조건은 스크립트 상단 `PLANS` 에 있다(새 취득결정이 나오면 여기 추가). 결과는
`10_국내주식/22_반도체/삼성전자_SK하이닉스_자사주_소진여력_2026-09-10.md` 의 추정치를 갈아 끼우는 데 쓴다.

```bash
~/.venv/krx/bin/python _scripts/자사주_소진여력_로컬.py                 # 오늘까지
~/.venv/krx/bin/python _scripts/자사주_소진여력_로컬.py --end 20261019 --pace-days 5
```

전제: 기타법인 = 회사 매수 라는 등식은 직접취득이고 다른 기타법인 매매가 없는 종목에서만 성립한다.
